(() => {
    ...
    function marqueeInit() {
        ...
        marqueeContainers.forEach(container => {
            const body = container.querySelector(".marquee__body");
            ...
            function move() {
                posX -= speed;
                if (Math.abs(posX) >= body.scrollWidth / 2) posX = 0;
                body.style.transform = `translate3d(${posX}px, 0, 0)`;
    ...
    closeForm?.addEventListener("click", function(e) {
        ...
        if (form) form.classList.add("form-close");
    });
    const closeThanks = document.querySelector(".fixed-form__close-thanks");
    closeThanks?.addEventListener("click", function(e) {
        const form = document.querySelector(".fixed-form");
        if (form) form.classList.remove("thanks");
    });
    const form = document.querySelector(".fixed-form");
    const requiredGroups = [ "quantity", "date", "time" ];
    ...
    async function formSend(e) {
        ...
        requiredGroups.forEach(groupName => {
            ...
            const parentSpoller = radios[0]?.closest(".spollers__item");
            const isChecked = Array.from(radios).some(r => r.checked);
            if (!isChecked) {
                hasErrors = true;
                parentSpoller?.classList.add("error");
            } else parentSpoller?.classList.remove("error");
        });
    ...
    form.querySelectorAll('input[type="radio"]').forEach(radio => {
        radio.addEventListener("change", () => {
            const parentSpoller = radio.closest(".spollers__item");
            parentSpoller?.classList.remove("error");
        });
    });
    const phoneInput = form.querySelector('input[type="tel"]');
